...
const LAUNCHKEY_MINI_MK2 = controllers.launchkey_mini_mk2
...
class Novation extends EventEmitter {
	constructor(options = {}) {
		...
		this.outputs = {
			...
			encoders: {
				midi: new EncodeStream({ useRunningStatus: this.useRunningStatus }),
				incontrol: new EncodeStream({ useRunningStatus: this.useRunningStatus }),
			},
		}
	...
	connect = () => {
		...
		Object.keys(this.controller.ports).map((portType) => {
    ...
    intensityToVelocity = (r, g, b) => {
        ...
        const color =
            this.controller.colors.red[rIdx] + this.controller.colors.green[gIdx]
        return color
	}
